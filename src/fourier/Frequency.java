...
/**
 * 
 * Performs a kind of edge detection
 * 
 ...
 *
 */
public class Frequency extends FourierOperation 
{
	//Do not modify variables after they have been set
	private double lowCutoff;
	private double highCutoff;
	public Frequency(double lowCutoff, double highCutoff)
	{
		super();
	...
	void run(double[][] im) 
	{
    	...
    	double lowCutoff = Math.pow(this.lowCutoff, 2.0);
    	double highCutoff = Math.pow(this.highCutoff, 2.0);
 		
    	for(int x = 0; x < im.length; x++)
        {
        	for(int y = 0; y < (im[0].length/2); y++)
        	{
        		double distance = Math.pow((x - centerX), 2) + Math.pow((y - centerY), 2.0);
